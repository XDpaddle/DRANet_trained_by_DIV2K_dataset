...
def uint2tensor4(img):
    if img.ndim == 2:
        img = np.expand_dims(img, axis=2)
    return paddle.to_tensor(np.ascontiguousarray(img),dtype='float32').permute(2, 0, 1).div(255.).unsqueeze(0)
...
def uint2tensor3(img):
    ...
    return paddle.to_tensor(np.ascontiguousarray(img),dtype='float32').permute(2, 0, 1).div(255.)

# convert 2/3/4-dimensional torch tensor to uint
def tensor2uint(img):
    #img = img.data.squeeze().float().clamp_(0, 1).cpu().numpy()
    img = paddle.clip(paddle.to_tensor(img.squeeze(),dtype='float32'),0,1).numpy()
    if img.ndim == 3:
...
def single2tensor3(img):
    return paddle.to_tensor(np.ascontiguousarray(img),dtype='float32').permute(2, 0, 1)


# convert single (HxWxC) to 4-dimensional torch tensor
def single2tensor4(img):
    #return paddle.to_tensor(np.ascontiguousarray(img),dtype='float32').permute(2, 0, 1).unsqueeze(0)
    return paddle.transpose(paddle.to_tensor(np.ascontiguousarray(img),dtype='float32'),[2,0,1]).unsqueeze(0)
...
def single2tensor5(img):
    ...
    return paddle.to_tensor(np.ascontiguousarray(img),dtype='float32').permute(2, 0, 1, 3).unsqueeze(0)


def single32tensor5(img):
    #return torch.from_numpy(np.ascontiguousarray(img)).float().unsqueeze(0).unsqueeze(0)
    return paddle.to_tensor(np.ascontiguousarray(img),dtype='float32').unsqueeze(0).unsqueeze(0)

def single42tensor4(img):
    #return torch.from_numpy(np.ascontiguousarray(img)).permute(2, 0, 1, 3).float()
    return paddle.to_tensor(np.ascontiguousarray(img),dtype='float32').permute(2, 0, 1, 3)
...
def augment_img_tensor(img, mode=0):
    ...
    img_tensor = paddle.to_tensor(np.ascontiguousarray(img_np))
    if len(img_size) == 3:
...
def calculate_psnrb(img1, img2, border=0):
    ...
    img1 = paddle.to_tensor(img1).permute(2, 0, 1).unsqueeze(0) / 255.
    img2 = paddle.to_tensor(img2).permute(2, 0, 1).unsqueeze(0) / 255.
    total = 0
...
def imresize_np(img, scale, antialiasing=True):
    ...
    # process H dimension
    # symmetric copying
    img_aug = paddle.zeros(shape=[in_H + sym_len_Hs + sym_len_He, in_W, in_C],dtype='float32')
    img_aug.narrow(0, sym_len_Hs, in_H).copy_(img)

    sym_patch = img[:sym_len_Hs, :, :]
    inv_idx = paddle.arange(sym_patch.size(0) - 1, -1, -1).long()
    ...
    out_1 = paddle.zeros(shape=[out_H, in_W, in_C],dtype='float32')
    kernel_width = weights_H.size(1)
    for i in range(out_H):
        idx = int(indices_H[i][0])
        for j in range(out_C):
    ...
    # process W dimension
    # symmetric copying
    out_1_aug = paddle.zeros(shape=[out_H, in_W + sym_len_Ws + sym_len_We, in_C],dtype='float32')
    ...
    out_2 = paddle.zeros(shape=[out_H, out_W, in_C],dtype='float32')
    kernel_width = weights_W.size(1)
    for i in range(out_W):
        idx = int(indices_W[i][0])
        for j in range(out_C):
