...
def find_last_checkpoint(save_dir, net_type='G', pretrained_path=None):
    ...
    file_list = glob.glob(os.path.join(save_dir, '*_{}.pdparams'.format(net_type)))
    if file_list:
        iter_exist = []
        for file_ in file_list:
            iter_current = re.findall(r"(\d+)_{}.pdparams".format(net_type), file_)
            iter_exist.append(int(iter_current[0]))
        init_iter = max(iter_exist)
        init_path = os.path.join(save_dir, '{}_{}.pdparams'.format(init_iter, net_type))
    else:
        init_iter = 0
...
def test_pad(model, L, modulo=16, sf=1):
    ...
    L = paddle.nn.Pad2D((0, paddingRight, 0, paddingBottom))(L)
    E = model(L)
    E = E[..., :h*sf, :w*sf]
    return E
...
def test_split_fn(model, L, refield=32, min_size=256, sf=1, modulo=1):
    ...
    h, w = L.shape[-2:]
    if h*w <= min_size**2:
        L = paddle.nn.Pad2D((0, int(np.ceil(w/modulo)*modulo-w), 0, int(np.ceil(h/modulo)*modulo-h)))(L)
        E = model(L)
        E = E[..., :h*sf, :w*sf]
    else:
        ...
        b, c = Es[0].shape[:2]
        #E = paddle.zeros([b, c, sf * h, sf * w]).type_as(L)
        E = paddle.zeros([b, c, sf * h, sf * w],dtype=L.dtype)
...
def test_onesplit(model, L, refield=32, min_size=256, sf=1, modulo=1):
    ...
    Es = [model(Ls[i]) for i in range(4)]
    b, c = Es[0].shape[:2]
    E = paddle.zeros(shape=[b, c, sf * h, sf * w],dtype=L.dtype)
    E[..., :h//2*sf, :w//2*sf] = Es[0][..., :h//2*sf, :w//2*sf]
    E[..., :h//2*sf, w//2*sf:w*sf] = Es[1][..., :h//2*sf, (-w + w//2)*sf:]
    E[..., h//2*sf:h*sf, :w//2*sf] = Es[2][..., (-h + h//2)*sf:, :w//2*sf]
    E[..., h//2*sf:h*sf, w//2*sf:w*sf] = Es[3][..., (-h + h//2)*sf:, (-w + w//2)*sf:]
...
def test_x8(model, L, modulo=1, sf=1):
    ...
    output_cat = paddle.stack(E_list, axis=0)
...
if __name__ == '__main__':
    ...
    class Net(paddle.nn.Layer):
        def __init__(self, in_channels=3, out_channels=3):
            ...
            self.conv = paddle.nn.Conv2D(in_channels=in_channels, out_channels=out_channels, kernel_size=3, padding=1)
