...
def main():
    ...
    model_name = 'dranet_color'           # set denoiser model, 'drunet_gray' | 'drunet_color'
    #testset_name = 'color'               # set test set,  'bsd68' | 'cbsd68' | 'set12'
    testset_name ='Set20'
    ...
    for idx, img in enumerate(L_paths):
        ...
        img_name, ext = os.path.splitext(os.path.basename(img))#basename:最后的文件名；splitext:分开文件名和后缀
        ...
        if not x8 and img_L.shape[2]//8==0 and img_L.shape[3]//8==0:
            img_E = model(img_L)
        elif not x8 and (img_L.shape[2]//8!=0 or img_L.shape[3]//8!=0):
